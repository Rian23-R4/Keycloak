...
function display_help() {
  ...
  echo "--rmi                     Remove images. Type 'all' to remove all images."
  exit 0
}
...
function get_docker_file() {
  if [[ $1 == "local" ]]; then
    ...
    docker_file=./.docker/compose/uat/docker-compose.yml
  else
...
function docker_container_handler() {
  ...
  if [[ $1 == "start" ]]; then
    ...
    if [[ $3 == 0 ]]; then
      ...
      docker compose --env-file=.env --file="${docker_file}" up
    fi
  elif [[ $1 == "stop" ]]; then
    # stop docker service
    ...
    if [[ $4 == 1 ]]; then
      ...
      docker compose --env-file=.env --file="${docker_file}" down --remove-orphans
    elif [[ $6 == 1 ]]; then
      echo "removing images"
      docker compose --env-file=.env --file="${docker_file}" down --rmi all -v --remove-orphans
    else
      echo "removing docker service default"
...
docker_container_handler "${action}" "${environment}" "${attached_container}" "${remove_volumes}" "${remove_orphans}" "${remove_images}"
